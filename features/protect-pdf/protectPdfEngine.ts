...
async function outputPath(name: string, suffix: string): Promise<string> {
  ...
  return `${base}nexus-pdf-${Crypto.randomUUID()}-${safeBaseName(name)}-${suffix}.pdf`;
}

export async function protectPdfWithEngine(input: ProtectPdfInput, userPassword: string): Promise<ProtectPdfResult> {
