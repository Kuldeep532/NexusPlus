...
export type EncryptionResult = { uri: string; originalName: string };
export type NativeFileEncryptionBridge = { isAvailable(): boolean; lockFile(inputUri: string, outputUri: string, password: string): Promise<string>; unlockFile(inputUri: string, outputUri: string, password: string): Promise<string> };

export function getNativeFileEncryptionBridge(): NativeFileEncryptionBridge {
  const bridge = (globalThis as typeof globalThis & { NexusFileEncryption?: NativeFileEncryptionBridge }).NexusFileEncryption;
  if (!bridge) throw new Error('Native file-encryption engine is unavailable in this build.');
  let available = false;
  try { available = bridge.isAvailable(); } catch { available = false; }
