...
export function NoteForm({ categories, initial, saving, onSave, onDraftChange, onOpenDrawing }: {
  ...
  const submit=async()=>{
    ...
    setPersisting(true);
    try{
      const stableAttachments=await Promise.all(attachments.map(async(item)=>{
        if(item.uri.startsWith('data:')||item.uri.includes('/Nexus Plus/Notes/'))return item;
        const storedUri=await persistAttachment(item.uri,item.name ?? (item.kind==='AUDIO'?'audio-note.m4a':'attachment'));
        return {...item,uri:storedUri};
      }));
      await onSave({
        title:title.trim(),
        content:content.trim(),
        categoryId,
        kind:stableAttachments.length&&kind==='TEXT'?'MIXED':kind,
