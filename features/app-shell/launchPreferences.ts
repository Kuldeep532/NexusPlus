...
export type LaunchPreferences={launchTarget:'nexus-plus';homeDestination:HomeDestination;showGeetaNexusOnHome:boolean;showDiscoverOnHome:boolean};
const DEFAULT_PREFERENCES:LaunchPreferences={launchTarget:'nexus-plus',homeDestination:'nexus-home',showGeetaNexusOnHome:true,showDiscoverOnHome:false};
export async function readLaunchPreferences():Promise<LaunchPreferences>{try{const raw=await AsyncStorage.getItem(STORAGE_KEY);if(!raw)return DEFAULT_PREFERENCES;const v=JSON.parse(raw) as Partial<LaunchPreferences>;return{launchTarget:'nexus-plus',homeDestination:v.homeDestination==='spiritual-home'?'spiritual-home':v.homeDestination==='geeta-home'?'geeta-home':'nexus-home',showGeetaNexusOnHome:v.showGeetaNexusOnHome!==false,showDiscoverOnHome:false}}catch{return DEFAULT_PREFERENCES}}
