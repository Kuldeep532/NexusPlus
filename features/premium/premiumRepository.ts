...
async function rpc<T>(name:string, body:Record<string,unknown>):Promise<T>{
  return request<T>('/rest/v1/rpc/'+name,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(body)});
}
...
export async function getActivePremiumPlans():Promise<PremiumCatalogPlan[]>{
  const [baseRows, bundleRows] = await Promise.all([
    request<PlanRow[]>('/rest/v1/subscription_plans?select=plan_id,plan_code,plan_name,tier_level,description,price_inr,duration_days,blocks_ads,unlocks_premium_features,included_credits&is_active=eq.true&order=tier_level.asc,duration_days.asc',undefined,false),
    request<BundleRow[]>('/rest/v1/subscription_credit_bundles?select=bundle_id,bundle_code,bundle_name,duration_days,price_inr,included_credits,tier_level,blocks_ads,unlocks_premium_features,description&is_active=eq.true&order=price_inr.asc',undefined,false).catch(() => []),
  ]);

  const base = baseRows.map(r=>({
    id:String(r.plan_id),
    name:r.plan_name,
    amount:Number(r.price_inr),
    code:r.plan_code,
    tierLevel:r.tier_level,
    durationDays:r.duration_days,
    blocksAds:r.blocks_ads,
    unlocksPremiumFeatures:r.unlocks_premium_features,
    description:r.description??'',
    includedCredits:Number(r.included_credits??0),
  }));

  const bundles = bundleRows.map(r=>({
    id:'bundle:' + String(r.bundle_id),
    name:r.bundle_name,
    amount:Number(r.price_inr),
    code:r.bundle_code,
    tierLevel:r.tier_level,
...
export async function getPaymentSettings():Promise<PaymentSettings>{const rows=await request<Array<{setting_key:string;setting_value:string}>>('/rest/v1/payment_settings?select=setting_key,setting_value&is_active=eq.true');const m=Object.fromEntries(rows.map(r=>[r.setting_key,r.setting_value]));if(!m.upi_id)throw new Error('PAYMENT_NOT_CONFIGURED');return{upiId:m.upi_id,receiverName:m.payment_receiver_name??'Nexus Plus',instructions:m.payment_instructions??'Complete the UPI payment. Nexus Plus will verify the bank/payment notification automatically.',provider:m.payment_provider??'upi_sms'};}
