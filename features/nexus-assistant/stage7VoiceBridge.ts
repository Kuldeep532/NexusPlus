...
async function resolveSpokenVoice(role: VoiceRole, locale: string) { const profile = getAssistantVoiceProfile(role, locale); const installed = await getInstalledVoices().catch(() => []); return { profile, voice: installed.find((item) => item.id === profile.id) }; }
