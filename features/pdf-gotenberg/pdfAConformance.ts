...
const PDF_A_ROUTE = '/forms/pdfengines/pdfa';

function validatePath(path: string): void {
  if (!path || path.length > 4096 || /[\u0000\r\n]/.test(path)) throw new Error('Invalid PDF path.');
}

export async function convertPdfToPdfAWithGotenberg(inputPath: string, outputFilename: string, conformance = 'PDF/A-2b'): Promise<string> {
  validatePath(inputPath);
  const safeName = outputFilename.replace(/[^a-zA-Z0-9._-]/g, '_');
  const response = await fetch(`${GOTENBERG_BASE_URL}${PDF_A_ROUTE}`, {
    method: 'POST',
    body: (() => {
    ...
    headers: {
      'Gotenberg-Output-Filename': safeName.replace(/\.pdf$/i, ''),
      'Gotenberg-Pdf-A-Compliance': conformance,
    },
  });
  if (!response.ok) {
    const message = await response.text().catch(() => '');
    throw new Error(`Gotenberg PDF/A conversion failed (${response.status}).${message ? ` ${message.slice(0, 300)}` : ''}`);
  }
