...
export async function upsertReminder(item: ReminderItem): Promise<ReminderItem[]> { const current = await loadReminders(); const next = [item, ...current.filter((entry) => entry.id !== item.id)]; await saveReminders(next); return next; }
...
export async function updateReminder(id: string, patch: Partial<ReminderItem>): Promise<ReminderItem[]> { const next = (await loadReminders()).map((item) => item.id === id ? { ...item, ...patch } : item); await saveReminders(next); return next; }
