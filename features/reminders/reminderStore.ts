...
export async function upsertReminder(item: ReminderItem): Promise<ReminderItem[]> { const current = await loadReminders(); const next = [{ ...item, updatedAt: new Date().toISOString() }, ...current.filter((entry) => entry.id !== item.id)]; await saveReminders(next); return next; }
