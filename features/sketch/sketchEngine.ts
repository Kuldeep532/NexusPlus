...
export function createSketchDocument(width = 1024, height = 1024, background = '#FFFFFF'): SketchDocument {
  return { version: 3, width, height, background, strokes: [] };
}

export function createStroke(
  points: Point[],
  settings: { color: string; width: number; opacity: number; tool: ToolKind },
): Stroke {
  return {
...
function polylineMarkup(stroke: Stroke): string {
  if (stroke.points.length < 2 || stroke.tool === 'fill') return '';
  const points = stroke.points.map((point) => `${point.x},${point.y}`).join(' ');
  const dash = stroke.tool === 'pencil' ? ' stroke-dasharray="2 1"' : '';
  const blur = stroke.tool === 'brush' || stroke.tool === 'airbrush' ? ' filter="url(#soft)"' : '';
  const blend = stroke.tool === 'highlighter' ? ' style="mix-blend-mode:multiply"' : '';
  return `<polyline points="${points}" fill="none" stroke="${escapeXml(stroke.color)}" stroke-opacity="${stroke.opacity}" stroke-width="${stroke.width}" stroke-linecap="round" stroke-linejoin="round"${dash}${blur}${blend} />`;
}
