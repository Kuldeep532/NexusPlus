...
import { buildPageOrder, pageRangeStrings, parsePageRanges } from './pdfPageInput';
...
export const PdfNativeBridge = {
  ...
  imageToPdf: async (inputPaths: string[], outputPath: string, quality = 90) => { validatePaths(inputPaths); validatePath(outputPath); return (await requireNative()).imageToPdf(inputPaths, outputPath, Math.max(1, Math.min(100, Math.round(quality)))); },
  pdfToImages: async (inputPath: string, outputDirectory: string, pageNumbers: number[], dpi = 300, format = 'png', pageCount?: number) => {
    ...
    if (!Array.isArray(pageNumbers) || !pageNumbers.length || pageNumbers.length > 1000) throw new Error('Select at least one valid PDF page.');
    const safePages = pageCount === undefined ? [...new Set(pageNumbers)] : parsePageRanges(pageNumbers.join(', '), pageCount).flatMap(({ start, end }) => Array.from({ length: end - start + 1 }, (_, i) => start + i));
    return (await requireNative()).pdfToImages(inputPath, outputDirectory, safePages, Math.max(72, Math.min(600, Math.round(Number(dpi) || 300))), validateImageFormat(format));
  },
  combineImages: async (inputPaths: string[], outputPath: string, format = 'png', quality = 95) => { validatePaths(inputPaths); validatePath(outputPath); if (inputPaths.length < 2) throw new Error('Select at least two images to combine.'); return (await requireNative()).combineImages(inputPaths, outputPath, validateImageFormat(format), Math.max(1, Math.min(100, Math.round(quality)))); },
  ...
  compress: async (inputPath: string, outputPath: string, quality = 75) => { validatePath(inputPath); validatePath(outputPath); return (await requireNative()).compress(inputPath, outputPath, Math.max(1, Math.min(100, Math.round(quality)))); },
  split: async (inputPath: string, outputDirectory: string, pageRanges: string[], pageCount?: number) => { validatePath(inputPath); validatePath(outputDirectory); const safeRanges = pageCount === undefined ? pageRanges : pageRangeStrings(pageRanges.join(', '), pageCount); return (await requireNative()).split(inputPath, outputDirectory, safeRanges); },
  reorder: async (inputPath: string, outputPath: string, pageOrder: number[], pageCount?: number) => { validatePath(inputPath); validatePath(outputPath); return (await requireNative()).reorder(inputPath, outputPath, pageCount === undefined ? pageOrder : buildPageOrder(pageCount, pageOrder)); },
