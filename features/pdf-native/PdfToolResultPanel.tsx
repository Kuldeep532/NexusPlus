...
export async function sharePdfResult(resultUri: string, title = 'Share PDF'): Promise<void> {
  try {
    if (!(await Sharing.isAvailableAsync())) {
      Alert.alert('Sharing unavailable', 'The PDF is already saved in Nexus Plus storage, but sharing is unavailable on this device.');
      return;
    }
    await Sharing.shareAsync(resultUri, { mimeType: 'application/pdf', dialogTitle: title });
  } catch {
    Alert.alert('Share unavailable', 'The saved PDF could not be shared.');
  }
}

export function PdfToolResultPanel({ resultUri, title = 'PDF saved successfully', filename, onClose, onReset }: Props) {
  ...
  return (
    <View accessibilityViewIsModal style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border }]}> 
      ...
      <Text accessibilityLiveRegion="polite" style={[styles.detail, { color: colors.mutedForeground }]}>{filename ?? 'Processed PDF'} has been saved to Nexus Plus → PDF Tools.</Text>
      <View style={styles.actions}>
        <Pressable accessibilityRole="button" accessibilityLabel="Share saved PDF" onPress={() => void sharePdfResult(resultUri, 'Share saved PDF')} style={[styles.action, { backgroundColor: colors.primary }]}> <Feather name="share-2" size={18} color={colors.primaryForeground} /><Text style={[styles.actionText, { color: colors.primaryForeground }]}>Share</Text></Pressable>
