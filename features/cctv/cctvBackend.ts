...
export const cctvCredentialStore: CctvCredentialStore = { async save(cameraId, username, password) { ensureNonEmpty(cameraId, 'Camera ID'); ensureNonEmpty(username, 'Username'); validatePassword(password, 'Password'); await SecureStore.setItemAsync(`${SECRET_PREFIX}${cameraId}`, JSON.stringify({ username, password }), CREDENTIAL_OPTIONS); }, async read(cameraId) { ensureNonEmpty(cameraId, 'Camera ID'); const raw = await SecureStore.getItemAsync(`${SECRET_PREFIX}${cameraId}`, CREDENTIAL_OPTIONS); if (!raw) return null; try { const parsed = JSON.parse(raw) as { username?: unknown; password?: unknown }; if (typeof parsed.username !== 'string' || typeof parsed.password !== 'string') return null; return { username: parsed.username, password: parsed.password }; } catch { return null; } }, async withCredentials(cameraId, operation) { const credentials = await this.read(cameraId); if (!credentials) throw new CctvBackendError({ code: 'AUTH_REQUIRED', message: 'Camera credentials are unavailable.', retryable: false }); return operation(credentials); }, async remove(cameraId) { ensureNonEmpty(cameraId, 'Camera ID'); await SecureStore.deleteItemAsync(`${SECRET_PREFIX}${cameraId}`); } };
...
export function sanitizeNetworkField(value: string | undefined): string | undefined { const normalized = value?.trim(); return normalized ? normalized : undefined; }
