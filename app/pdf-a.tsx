...
import { convertPdfToPdfAWithGotenberg } from '@/features/pdf-gotenberg/pdfAConformance';
...
export default function PdfAScreen() {
  ...
  const [conformance] = useState('PDF/A-2b');
  ...
  async function convert() {
    ...
    setBusy(true); setResult(null); setStatus(`Converting PDF to ${conformance}…`);
    try {
  ...
  return <View style={[styles.root, { backgroundColor: colors.background }]}>
    <Stack.Screen options={{ title: 'PDF/A Converter' }} />
    {busy ? <View style={styles.loading}><ActivityIndicator size="large" color={colors.primary} /><Text accessibilityRole="header" style={[styles.loadingTitle, { color: colors.foreground }]}>Converting PDF</Text><Text accessibilityLiveRegion="polite" style={[styles.loadingText, { color: colors.mutedForeground }]}>{status}</Text></View> : <ScrollView contentContainerStyle={{ paddingTop: insets.top + 18, paddingBottom: insets.bottom + 40 }} showsVerticalScrollIndicator={false}>
      <View style={styles.header}><View style={[styles.icon, { backgroundColor: colors.secondary }]}><MaterialCommunityIcons name="archive-arrow-down-outline" size={29} color={colors.primary} /></View><View style={styles.copy}><Text accessibilityRole="header" style={[styles.title, { color: colors.foreground }]}>PDF/A Converter</Text><Text style={[styles.subtitle, { color: colors.mutedForeground }]}>Convert a PDF into an archival PDF/A document using the existing Gotenberg PDF Engine flow.</Text></View></View>
      ...
      {!!file && !result && <View style={[styles.info, { backgroundColor: colors.card, borderColor: colors.border }]}><Text style={[styles.infoLabel, { color: colors.mutedForeground }]}>CONFORMANCE</Text><Text style={[styles.infoValue, { color: colors.foreground }]}>{conformance}</Text></View>}
      {!result && <Pressable accessibilityRole="button" accessibilityLabel="Convert PDF to PDF/A" onPress={() => void convert()} disabled={!file} style={({ pressed }) => [styles.primary, { backgroundColor: colors.primary }, (!file || pressed) && styles.disabled]}><MaterialCommunityIcons name="archive-arrow-down-outline" size={19} color={colors.primaryForeground} /><Text style={[styles.primaryText, { color: colors.primaryForeground }]}>Convert to PDF/A</Text></Pressable>}
      ...
      <PdfToolResultPanel resultUri={result} filename={result ? `${safeBaseName(file?.name || 'document')}-pdf-a.pdf` : undefined} mimeType="application/pdf" onClose={reset} onReset={reset} title="PDF/A conversion completed" />
    </ScrollView>}
  </View>;
}
const styles = StyleSheet.create({ root: { flex: 1 }, header: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 20, marginBottom: 20 }, icon: { width: 54, height: 54, borderRadius: 16, alignItems: 'center', justifyContent: 'center' }, copy: { flex: 1, marginLeft: 13 }, title: { fontSize: 28, fontFamily: 'Inter_700Bold', marginBottom: 5 }, subtitle: { fontSize: 12, lineHeight: 18 }, pick: { marginHorizontal: 20, marginTop: 14, minHeight: 72, borderWidth: 1, borderRadius: 16, paddingHorizontal: 14, flexDirection: 'row', alignItems: 'center' }, pickCopy: { flex: 1, marginHorizontal: 12 }, pickTitle: { fontSize: 13, fontFamily: 'Inter_700Bold', marginBottom: 4 }, pickDetail: { fontSize: 11 }, info: { marginHorizontal: 20, marginTop: 14, borderWidth: 1, borderRadius: 14, padding: 12 }, infoLabel: { fontSize: 9, letterSpacing: 1.2, fontFamily: 'Inter_700Bold', marginBottom: 3 }, infoValue: { fontSize: 12, fontFamily: 'Inter_600SemiBold' }, primary: { marginHorizontal: 20, marginTop: 16, minHeight: 52, borderRadius: 15, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 9 }, primaryText: { fontSize: 14, fontFamily: 'Inter_700Bold' }, status: { marginHorizontal: 20, marginTop: 16, fontSize: 11, lineHeight: 17 }, loading: { flex: 1, alignItems: 'center', justifyContent: 'center', paddingHorizontal: 30 }, loadingTitle: { fontSize: 22, fontFamily: 'Inter_700Bold', marginTop: 22, marginBottom: 8 }, loadingText: { fontSize: 13, lineHeight: 19, textAlign: 'center' }, pressed: { opacity: 0.75 }, disabled: { opacity: 0.5 } });
