...
export default function ExtractPdfPagesScreen() {
  ...
  function resetTool() {
    void cleanupPdfToolResult(result);
    setPdf(null); setSelection(''); setResult(null); setStatus('');
  }
  ...
  async function extract() {
    ...
    try {
      ...
      setResult(uri);
      setStatus('Pages cut and extracted successfully.');
    } catch (error) {
      setStatus(error instanceof Error ? error.message : 'Could not cut and extract the selected pages.');
    } finally {
      setBusy(false);
    }
  }

  useEffect(() => () => { void cleanupPdfToolResult(result); }, [result]);
    ...
    <View style={[styles.root, { backgroundColor: colors.background }]}> 
      ...
      {busy ? (
        <View style={styles.loading}>
          ...
          <Text accessibilityRole="header" style={[styles.loadingTitle, { color: colors.foreground }]}>Cutting & Extracting Pages</Text>
        ...
        <ScrollView contentContainerStyle={{ paddingTop: insets.top + 18, paddingBottom: insets.bottom + 40 }} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
          <View style={styles.header}>
            <View style={[styles.icon, { backgroundColor: colors.secondary }]}><MaterialCommunityIcons name="content-cut" size={28} color={colors.primary} /></View>
            <View style={styles.copy}><Text accessibilityRole="header" style={[styles.title, { color: colors.foreground }]}>Cut & Extract Pages</Text><Text style={[styles.subtitle, { color: colors.mutedForeground }]}>Cut selected pages from a PDF and save them as a new PDF.</Text></View>
          </View>
          ...
          {pdf && !result && <>
            <Text style={[styles.label, { color: colors.foreground }]}>Pages to cut & extract</Text>
            ...
            <Text style={[styles.hint, { color: colors.mutedForeground }]}>Only 1-{pdf.pageCount} are accepted. Numbers above {pdf.pageCount} are automatically clamped. Example: 2, 4-6.</Text>
            <Pressable accessibilityRole="button" accessibilityLabel="Cut and extract selected PDF pages" onPress={() => void extract()} disabled={!selection.trim()} style={({ pressed }) => [styles.primary, { backgroundColor: colors.primary }, (pressed || !selection.trim()) && styles.disabled]}>
              <MaterialCommunityIcons name="content-cut" size={19} color={colors.primaryForeground} /><Text style={[styles.primaryText, { color: colors.primaryForeground }]}>Cut & Extract Pages</Text>
            </Pressable>
          </>}
          ...
          <PdfToolResultPanel
            resultUri={result}
            filename={result ? `${safeBaseName(pdf?.name ?? 'document')}-cut-extracted.pdf` : undefined}
            onClose={resetTool}
            onReset={resetTool}
            title="Cut & extract completed"
          />
        </ScrollView>
      )}
    </View>
