...
const MODES: Array<{ id: CalculatorMode; label: string; icon: string }> = [
  { id: 'standard', label: 'Normal', icon: 'hash' },
  { id: 'age', label: 'Age', icon: 'user' },
  { id: 'tip-bill', label: 'Tip & Bill', icon: 'users' },
  { id: 'electricity', label: 'Electricity', icon: 'zap' },
  { id: 'fixed-deposit', label: 'Fixed Deposit', icon: 'briefcase' },
...
function useAnnouncer() {
  return (text: string) => { void Speech.stop().then(() => Speech.speak(text, { rate: 0.9 })); };
}
...
export default function CalculatorScreen() {
  ...
  const onStandard = (key: string) => {
    if (key === '=') {
      try { speakResult(formatNumber(evaluateStandard(expression))); } catch (error) { const message = error instanceof Error ? error.message : 'Calculation error.'; Alert.alert('Calculator', message); announce(message); }
      return;
    }
    if (key === 'C') { setExpression(''); setResult(''); announce('Cleared'); return; }
    ...
    <ScrollView contentContainerStyle={[styles.content, { paddingTop: insets.top + 8, paddingBottom: insets.bottom + 30 }]}>
      ...
      {mode === 'standard' ? <View style={styles.standardWrap}>
        ...
        <View style={[styles.form, { backgroundColor: colors.card, borderColor: colors.border }]}>
          {mode === 'age' && field('dob', 'Date of birth', 'YYYY-MM-DD')}
          {mode === 'tip-bill' && <>{field('bill','Bill amount','1000')}{field('tip','Tip percentage','10')}{field('people','Number of people','2')}</>}
          {mode === 'electricity' && <>{field('units','Units consumed','120')}{field('fixed','Fixed charge','100')}{field('rate','Energy rate per unit','8')}{field('surcharge','Surcharge percentage','0')}</>}
          {mode === 'fixed-deposit' && <>{field('principal','Principal','100000')}{field('rate','Annual interest rate','7')}{field('years','Tenure in years','5')}{field('compound','Compounds per year','4')}</>}
          {mode === 'emi' && <>{field('loan','Loan amount','500000')}{field('rate','Annual interest rate','9')}{field('months','Tenure in months','60')}</>}
          {mode === 'discount' && <>{field('price','Marked price','2000')}{field('discount','Discount percentage','15')}{field('tax','Tax percentage','18')}</>}
          {mode === 'percentage' && <>{field('number','Number','500')}{field('percent','Percentage','12')}</>}
          <Pressable accessibilityRole="button" accessibilityLabel="Calculate and speak result" onPress={calculate} style={[styles.calculateButton, { backgroundColor: colors.primary }]}><Feather name="volume-2" size={18} color={colors.primaryForeground} /><Text style={[styles.calculateText, { color: colors.primaryForeground }]}>Calculate & Speak</Text></Pressable>
        </View>
