...
export default function NotepadRoute(){
 ...
 const [screen,setScreen]=useState<Screen>('home'); const [notes,setNotes]=useState<Note[]>([]); const [categories,setCategories]=useState<NoteCategory[]>([]);
 const [editing,setEditing]=useState<Note|undefined>(); const [search,setSearch]=useState(''); const [selectedCategory,setSelectedCategory]=useState('');
 const [saving,setSaving]=useState(false); const [newCategory,setNewCategory]=useState(''); const [draftDirty,setDraftDirty]=useState(false); const [pendingDraft,setPendingDraft]=useState<NoteDraft|null>(null);

 const load=useCallback(async()=>{const [loadedNotes,loadedCategories]=await Promise.all([localNotesRepository.listNotes(),localNotesRepository.listCategories()]);if(loadedCategories.length===0){const now=Date.now();const defaults=DEFAULT_NOTE_CATEGORIES.map(item=>({...item,createdAt:now,updatedAt:now}));for(const category of defaults)await localNotesRepository.saveCategory(category);setCategories(defaults);}else setCategories(loadedCategories);setNotes(loadedNotes);},[]);
 useEffect(()=>{void load();},[load]);

 const visibleNotes=notes.filter(note=>{const q=search.trim().toLowerCase();return !note.archived&&(!q||(note.title+' '+note.content+' '+note.kind).toLowerCase().includes(q))&&(!selectedCategory||note.categoryId===selectedCategory);});

 const saveDraft=async(draft:NoteDraft)=>{
  setSaving(true);
  try{
   const now=Date.now();
   const note:Note=editing?{...editing,kind:draft.kind,title:draft.title,content:draft.content,categoryId:draft.categoryId,attachments:draft.attachments,updatedAt:now}:{id:now.toString(36)+'-'+Math.random().toString(36).slice(2,8),kind:draft.kind,title:draft.title,content:draft.content,categoryId:draft.categoryId,attachments:draft.attachments,createdAt:now,updatedAt:now,source:'NOTEPAD'};
   await localNotesRepository.saveNote(note);setNotes(current=>[note,...current.filter(item=>item.id!==note.id)]);
