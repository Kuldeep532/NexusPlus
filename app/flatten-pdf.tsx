...
export default function FlattenPdfScreen() {
  ...
  return (
    <View style={[styles.root, { backgroundColor: colors.background }]}>
      <Stack.Screen options={{ title: 'Flatten PDF' }} />
      {busy ? (
        <View style={styles.loading}>
          <ActivityIndicator size="large" color={colors.primary} />
          <Text accessibilityRole="header" style={[styles.loadingTitle, { color: colors.foreground }]}>Flattening PDF</Text>
        ...
        <ScrollView contentContainerStyle={{ paddingTop: insets.top + 18, paddingBottom: insets.bottom + 40 }} showsVerticalScrollIndicator={false}>
          ...
          <Pressable accessibilityRole="button" accessibilityLabel="Choose PDF" accessibilityHint="Select the PDF file to flatten." onPress={() => void pickFile()} style={({ pressed }) => [styles.pick, { backgroundColor: colors.card, borderColor: colors.border }, pressed && styles.pressed]}>
          ...
          {!!file && !result && (
            <View style={[styles.info, { backgroundColor: colors.card, borderColor: colors.border }]}>
              ...
              <Text style={[styles.infoValue, { color: colors.foreground }]}>Flattened PDF</Text>
            </View>
          )}

          {!result && (
            <Pressable accessibilityRole="button" accessibilityLabel="Flatten PDF" onPress={() => void flatten()} disabled={!file} style={({ pressed }) => [styles.primary, { backgroundColor: colors.primary }, (!file || pressed) && styles.disabled]}>
              <MaterialCommunityIcons name="layers-triple-outline" size={19} color={colors.primaryForeground} />
              <Text style={[styles.primaryText, { color: colors.primaryForeground }]}>Flatten PDF</Text>
            </Pressable>
          )}

          <Text accessibilityLiveRegion="polite" style={[styles.note, { color: colors.mutedForeground }]}>Flattening makes interactive form fields and annotations part of the page content, so they are no longer editable.</Text>
          ...
          <PdfToolResultPanel resultUri={result} filename={result ? `${safeBaseName(file?.name || 'document')}-flattened.pdf` : undefined} mimeType="application/pdf" onClose={reset} onReset={reset} title="PDF flattened successfully" />
        </ScrollView>
      )}
    </View>
  );
