...
export default function KaraokeScreen() {
  ...
  const togglePause = useCallback(async () => {
    ...
    try {
      if (pausedRef.current) {
        await resumeKaraokeRecording();
        player.play();
        pausedRef.current = false;
        setPaused(false);
        setMessage(recordingRef.current ? 'Recording and karaoke resumed.' : 'Karaoke resumed.');
      } else {
        if (recordingRef.current) await pauseKaraokeRecording();
        player.pause();
        pausedRef.current = true;
        setPaused(true);
        setMessage(recordingRef.current ? 'Recording and karaoke paused.' : 'Karaoke paused.');
      }
    } catch (error) {
      setMessage(error instanceof Error ? error.message : 'Unable to change karaoke pause state.');
    } finally {
      setWorking(false);
    }
  }, [player, working]);
  ...
  const finishSinging = useCallback(async () => {
    ...
    try {
      ...
      setRecording(false);
      setOutput({ path: result.outputPath, uri: `file://${result.outputPath}` });
      setMessage('Recording finished and saved.');
    } catch (error) {
        ...
        {track && <View style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
          ...
          {!singing ? (
            <View style={styles.transportRow}>
              <Pressable onPress={() => void startSinging(false)} disabled={working} accessibilityRole="button" style={[styles.transportButton, { backgroundColor: colors.primary }]}><Feather name="play" size={19} color={colors.primaryForeground} /><Text style={[styles.buttonText, { color: colors.primaryForeground }]}>Start Singing</Text></Pressable>
              <Pressable onPress={() => void startSinging(true)} disabled={working} accessibilityRole="button" style={[styles.transportButton, { borderColor: colors.primary, borderWidth: 1 }]}><Feather name="mic" size={18} color={colors.primary} /><Text style={[styles.modeText, { color: colors.primary }]}>Start Recording</Text></Pressable>
            </View>
          ) : (
            <View style={styles.transportRow}>
              <Pressable onPress={() => void togglePause()} disabled={working} accessibilityRole="button" accessibilityLabel={recording ? (paused ? 'Resume Recording' : 'Pause Recording') : (paused ? 'Resume Singing' : 'Pause Singing')} style={[styles.transportButton, { backgroundColor: colors.primary }]}><Feather name={paused ? 'play' : 'pause'} size={18} color={colors.primaryForeground} /><Text style={[styles.buttonText, { color: colors.primaryForeground }]}>{recording ? (paused ? 'Resume Recording' : 'Pause Recording') : (paused ? 'Resume Singing' : 'Pause Singing')}</Text></Pressable>
              <Pressable onPress={() => void finishSinging()} disabled={working} accessibilityRole="button" style={[styles.transportButton, { borderColor: colors.primary, borderWidth: 1 }]}><Feather name="square" size={18} color={colors.primary} /><Text style={[styles.modeText, { color: colors.primary }]}>{recording ? 'Finish Recording' : 'End Singing'}</Text></Pressable>
            </View>
          )}
