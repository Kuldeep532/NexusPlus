...
const TARGETS: Array<{ id: AudioNormalizationTarget; title: string; description: string }> = [
  ...
  { id: 0.85, title: 'Balanced', description: 'Peak target around 85% for a strong general-purpose export.' },
  ...
  return (
    <ScrollView style={[styles.root, { backgroundColor: colors.background }]} contentContainerStyle={{ padding: 18, paddingTop: insets.top + 12, paddingBottom: insets.bottom + 34 }}>
      ...
      <View style={styles.headerRow}>
        <View style={[styles.heroIcon, { backgroundColor: colors.secondary }]}><Feather name="bar-chart-2" size={24} color={colors.primary} /></View>
        <View style={styles.headerCopy}><Text accessibilityRole="header" style={[styles.title, { color: colors.foreground }]}>Audio Normalizer</Text><Text style={[styles.subtitle, { color: colors.mutedForeground }]}>Measure the source peak and create a new copy at a controlled peak level.</Text></View>
      </View>

      {output ? <AudioEditorResultPanel message="Audio normalized and saved." outputPath={output.path} resultUri={output.uri} onClose={reset} /> : <>
        ...
        {source && <View style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
          ...
          <View style={styles.targetList}>{TARGETS.map((item) => <Pressable key={item.id} onPress={() => setTarget(item.id)} accessibilityRole="button" accessibilityState={{ selected: target === item.id }} style={[styles.target, { borderColor: target === item.id ? colors.primary : colors.border, backgroundColor: target === item.id ? colors.secondary : colors.background }]}><View style={styles.targetCopy}><Text style={[styles.targetTitle, { color: colors.foreground }]}>{item.title} · {Math.round(item.id * 100)}%</Text><Text style={[styles.meta, { color: colors.mutedForeground }]}>{item.description}</Text></View>{target === item.id && <Feather name="check-circle" size={19} color={colors.primary} />}</Pressable>)}</View>
          <Pressable onPress={generate} disabled={working} accessibilityRole="button" accessibilityState={{ disabled: working }} style={[styles.primaryButton, { marginTop: 4, backgroundColor: working ? colors.muted : colors.primary }]}>{working ? <ActivityIndicator color={colors.primaryForeground} /> : <Feather name="volume-2" size={19} color={colors.primaryForeground} />}<Text style={[styles.buttonText, { color: colors.primaryForeground }]}>{working ? 'Generating…' : 'Normalize Audio'}</Text></Pressable>
        </View>}
      </>}
