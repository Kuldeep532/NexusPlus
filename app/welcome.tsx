  ...
  return (
    <View style={[styles.root, { backgroundColor: colors.background, paddingTop: insets.top, paddingBottom: insets.bottom }]}> 
      <View style={styles.hero}>
        ...
        <Text style={[styles.subtitle, { color: colors.mutedForeground }]}>One secure home for your everyday tools, utilities and financial features.</Text>
      </View>

      <View style={styles.actions}>
        <Pressable accessibilityRole="button" accessibilityLabel="Continue to login" onPress={() => router.push('/login-plus-register')} style={[styles.primaryButton, { backgroundColor: colors.primary }]}>
          <Text style={[styles.primaryText, { color: colors.primaryForeground }]}>Get Started</Text>
        </Pressable>
        <Text style={[styles.securityNote, { color: colors.mutedForeground }]}>Your account and protected features use the app's existing secure authentication system.</Text>
      </View>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  subtitle: { maxWidth: 340, fontSize: 13, lineHeight: 20, textAlign: 'center' },
