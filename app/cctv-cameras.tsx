...
export default function CctvCamerasScreen() {
  ...
  const refresh = useCallback(async () => {
    setLoading(true);
    try { setState(await getCctvManagementState()); } finally { setLoading(false); }
  }, []);
      ...
      <ScrollView contentContainerStyle={[styles.content, { paddingTop: insets.top + 10, paddingBottom: insets.bottom + 28 }]} accessibilityLabel="CCTV camera management">
        ...
        {loading ? <Text style={[styles.message, { color: colors.mutedForeground }]}>Loading CCTV cameras…</Text> : cameras.length === 0 ? (
          ...
          <>
                ...
                <View key={camera.id} style={[styles.cameraCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
                  <Pressable accessibilityRole="button" accessibilityLabel={`Open ${camera.name}`} onPress={() => router.push({ pathname: '/cctv-live-view', params: { cameraId: camera.id } })} style={styles.cameraLink}>
                    ...
                    <View style={styles.cameraCopy}>
                      ...
                      <Text style={[styles.meta, { color: colors.mutedForeground }]}>{(camera.deviceKind ?? 'ip_camera').replace('_', ' ').toUpperCase()} · {camera.manufacturer ?? 'Authorized device'}</Text>
                      <Text style={[styles.meta, { color: colors.mutedForeground }]}>{camera.serialNumber ? `Serial ending ${camera.serialNumber.slice(-4)}` : 'Authorized camera'}</Text>
                    </View>
            ...
            <Pressable accessibilityRole="button" accessibilityLabel="Add another CCTV camera securely" onPress={() => router.push('/cctv-add')} style={[styles.secondaryButton, { borderColor: colors.border, backgroundColor: colors.card }]}>
              <Feather name="plus-circle" size={18} color={colors.primary} />
              <Text style={[styles.secondaryText, { color: colors.foreground }]}>Add Another CCTV</Text>
            </Pressable>
          </>
        )}
      </ScrollView>
...
function AddButton({ colors, onPress }: { colors: ReturnType<typeof useColors>; onPress: () => void }) {
  return <Pressable accessibilityRole="button" accessibilityLabel="Securely add CCTV camera" onPress={onPress} style={[styles.addButton, { backgroundColor: colors.primary }]}><Feather name="plus" size={21} color={colors.primaryForeground} /></Pressable>;
}

const styles = StyleSheet.create({
  root: { flex: 1 }, content: { paddingHorizontal: 18, gap: 12 }, headerRow: { flexDirection: 'row', alignItems: 'center', marginBottom: 6 }, copy: { flex: 1 }, title: { fontSize: 24, fontFamily: 'Inter_700Bold' }, subtitle: { fontSize: 11, lineHeight: 17, marginTop: 5 }, addButton: { width: 48, height: 48, borderRadius: 14, alignItems: 'center', justifyContent: 'center', marginLeft: 12 }, message: { fontSize: 12 }, empty: { borderWidth: 1, borderRadius: 18, padding: 22, alignItems: 'center', gap: 10 }, emptyTitle: { fontSize: 17, fontFamily: 'Inter_700Bold', textAlign: 'center' }, emptyText: { fontSize: 11, lineHeight: 17, textAlign: 'center' }, primaryButton: { minHeight: 50, paddingHorizontal: 18, borderRadius: 14, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 9, marginTop: 4 }, buttonText: { fontSize: 12, fontFamily: 'Inter_700Bold' }, list: { gap: 10 }, cameraCard: { minHeight: 90, borderWidth: 1, borderRadius: 18, padding: 12, flexDirection: 'row', alignItems: 'center' }, cameraLink: { flex: 1, flexDirection: 'row', alignItems: 'center', minWidth: 0 }, icon: { width: 46, height: 46, borderRadius: 14, alignItems: 'center', justifyContent: 'center' }, cameraCopy: { flex: 1, minWidth: 0, marginHorizontal: 11 }, name: { fontSize: 14, fontFamily: 'Inter_700Bold' }, meta: { fontSize: 10.5, lineHeight: 16, marginTop: 2 }, deleteButton: { width: 40, height: 40, alignItems: 'center', justifyContent: 'center' }, secondaryButton: { minHeight: 50, borderWidth: 1, borderRadius: 14, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8 }, secondaryText: { fontSize: 12, fontFamily: 'Inter_700Bold' }, });
