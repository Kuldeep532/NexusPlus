...
export default function ProductivityToolsScreen() {
  const colors = useColors();
  const router = useRouter();
  const insets = useSafeAreaInsets();
  const tools = getFeaturesByCategory('productivity');
      ...
      <View style={styles.list}>
        {tools.map((tool) => (
          <Pressable key={tool.id} accessibilityRole="button" accessibilityLabel={`${tool.title}. ${tool.description}`} onPress={() => router.push(tool.route as never)} style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
            <View style={[styles.icon, { backgroundColor: colors.secondary }]}><Feather name={tool.icon as never} size={20} color={colors.primary} /></View>
            <View style={styles.copy}><Text style={[styles.cardTitle, { color: colors.foreground }]}>{tool.title}</Text><Text style={[styles.body, { color: colors.mutedForeground }]}>{tool.description}</Text></View>
...
const styles = StyleSheet.create({
  ...
  card: { minHeight: 74, borderWidth: 1, borderRadius: 18, padding: 13, flexDirection: 'row', alignItems: 'center' },
