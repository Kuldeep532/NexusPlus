...
export default function DocumentStudioScreen() {
  ...
  return (
    <View style={[styles.root, { backgroundColor: colors.background }]}>
      ...
      <ScrollView contentContainerStyle={{ paddingTop: insets.top + 14, paddingBottom: insets.bottom + 36 }} showsVerticalScrollIndicator={false}>
        <View style={styles.header}>
          <View style={[styles.icon, { backgroundColor: colors.secondary }]}>
            <MaterialCommunityIcons name="file-document-multiple-outline" size={31} color={colors.primary} />
          </View>
          <View style={styles.copy}>
            <Text accessibilityRole="header" style={[styles.title, { color: colors.foreground }]}>Document Studio</Text>
            <Text style={[styles.subtitle, { color: colors.mutedForeground }]}>Create, scan, read, convert and manage documents from dedicated tools.</Text>
          </View>
        </View>

        <Text style={[styles.section, { color: colors.foreground }]}>Document tools</Text>
        <View style={styles.list}>
          {DOCUMENT_STUDIO_TOOLS.map((tool) => (
            <Pressable
              key={tool.id}
            ...
            >
              <View style={[styles.toolIcon, { backgroundColor: colors.secondary }]}>
                <Feather name={tool.icon as never} size={21} color={colors.primary} />
              </View>
...
const styles = StyleSheet.create({
  ...
  section: { marginHorizontal: 20, fontSize: 16, fontFamily: 'Inter_700Bold', marginBottom: 10 },
  list: { marginHorizontal: 20, gap: 10 },
  card: { minHeight: 78, borderWidth: 1, borderRadius: 18, padding: 13, flexDirection: 'row', alignItems: 'center' },
