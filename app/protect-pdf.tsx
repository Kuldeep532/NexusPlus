...
export default function ProtectPdfScreen() {
  ...
  const pickPdf = async () => {
    setStatus(''); setResult(null); setVaultSaveAvailable(false);
    ...
    setPassword(''); setConfirmPassword(''); setState('selecting');
  ...
  const protect = async () => {
    ...
    setState('processing'); setStatus('Protecting PDF…'); setResult(null);
    try {
      const protectedPdf = await protectPdfWithEngine(pdf, password);
      setResult(protectedPdf); setVaultSaveAvailable(true); setState('completed');
      setStatus('PDF protected successfully. Save the password to Nexus Vault now if needed.');
    } catch {
      setState('error'); setStatus('Could not protect this PDF.'); setPassword(''); setConfirmPassword(''); setVaultSaveAvailable(false);
    }
  };

  const share = async () => {
    if (!result || !(await Sharing.isAvailableAsync())) return;
    try { await Sharing.shareAsync(result.uri, { mimeType: 'application/pdf', dialogTitle: 'Share protected PDF' }); }
    catch { Alert.alert('Share unavailable', 'The protected PDF could not be shared.'); }
  };
  ...
  const savePassword = async () => {
    ...
    try {
      ...
      setPassword(''); setConfirmPassword(''); setVaultSaveAvailable(false);
    } catch { Alert.alert('Could not save password', 'The encrypted Nexus Vault could not be updated. Your PDF was not changed.'); }
  };

  const clearAndRegenerate = () => {
    setPdf(null); setResult(null); setPassword(''); setConfirmPassword(''); setVaultSaveAvailable(false); setStatus(''); setState('idle');
  };
  const onBack = () => { clearAndRegenerate(); router.back(); };
    ...
    <View style={[styles.root, { backgroundColor: colors.background }]}> 
      ...
      {state === 'processing' ? (
        <View style={[styles.processing, { paddingTop: insets.top, paddingBottom: insets.bottom }]}> 
          <View style={[styles.processingIcon, { backgroundColor: colors.secondary }]}><ActivityIndicator size="large" color={colors.primary} /></View>
        ...
        <ScrollView style={styles.scroll} contentContainerStyle={{ paddingTop: insets.top + 18, paddingBottom: insets.bottom + 40 }} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
          <View style={styles.header}>
            <View style={[styles.icon, { backgroundColor: colors.secondary }]}><MaterialCommunityIcons name="shield-lock" size={28} color={colors.primary} /></View>
            <View style={styles.headerCopy}><Text accessibilityRole="header" style={[styles.title, { color: colors.foreground }]}>Protect PDF</Text><Text style={[styles.subtitle, { color: colors.mutedForeground }]}>Add password protection before you share or save a sensitive document.</Text></View>
          </View>
          <Pressable accessibilityRole="button" accessibilityLabel={pdf ? `Selected PDF ${pdf.name}` : 'Choose PDF'} onPress={pickPdf} disabled={state === 'processing'} style={({ pressed }) => [styles.pick, { backgroundColor: colors.card, borderColor: colors.border }, pressed && styles.pressed]}>
            <Feather name="file-plus" size={20} color={colors.primary} />
            <View style={styles.pickCopy}><Text style={[styles.pickTitle, { color: colors.foreground }]}>{pdf ? pdf.name : 'Choose PDF'}</Text><Text style={[styles.pickDetail, { color: colors.mutedForeground }]}>{pdf ? 'Ready to protect' : 'Select a local PDF file'}</Text></View>
          ...
          {pdf && state !== 'completed' && state !== 'error' && <>
            ...
            <Pressable accessibilityRole="button" accessibilityLabel="Protect PDF" onPress={protect} disabled={!password || !confirmPassword} style={({ pressed }) => [styles.primaryButton, { backgroundColor: colors.primary }, (pressed || !password || !confirmPassword) && styles.disabled]}><MaterialCommunityIcons name="shield-lock" size={19} color={colors.primaryForeground} /><Text style={[styles.primaryText, { color: colors.primaryForeground }]}>Protect PDF</Text></Pressable>
          </>}
          ...
          {state === 'completed' && result && <View style={styles.actions}>
            <Pressable accessibilityRole="button" accessibilityLabel="Share protected PDF" onPress={share} style={[styles.actionButton, { borderColor: colors.border, backgroundColor: colors.card }]}><Feather name="share-2" size={18} color={colors.primary} /><Text style={[styles.actionText, { color: colors.foreground }]}>Share</Text></Pressable>
            <Pressable accessibilityRole="button" accessibilityLabel="Save password to Nexus Vault" onPress={savePassword} disabled={!vaultSaveAvailable} style={[styles.actionButton, { borderColor: colors.border, backgroundColor: colors.card }, !vaultSaveAvailable && styles.disabled]}><MaterialCommunityIcons name="shield-key" size={19} color={colors.primary} /><Text style={[styles.actionText, { color: colors.foreground }]}>Save Password to Nexus Vault</Text></Pressable>
            <Pressable accessibilityRole="button" accessibilityLabel="Protect another PDF" onPress={clearAndRegenerate} style={[styles.actionButton, { borderColor: colors.border, backgroundColor: colors.card }]}><Feather name="refresh-cw" size={18} color={colors.primary} /><Text style={[styles.actionText, { color: colors.foreground }]}>Protect Another</Text></Pressable>
          </View>}
          {(state === 'error' || state === 'completed') && <Pressable accessibilityRole="button" accessibilityLabel="Go back to PDF tools" onPress={onBack} style={[styles.secondaryButton, { borderColor: colors.border, backgroundColor: colors.card }]}><Feather name="arrow-left" size={18} color={colors.foreground} /><Text style={[styles.actionText, { color: colors.foreground }]}>Back to PDF Tools</Text></Pressable>}
        </ScrollView>
      )}
    </View>
  );
