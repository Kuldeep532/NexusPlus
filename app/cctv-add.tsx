...
export default function CctvAddScreen() {
  ...
  const [draft, setDraft] = useState<CctvSetupDraft>(initialCctvSetupDraft()); const [deviceKind, setDeviceKind] = useState<CctvDeviceKind>('ip_camera');
  const [permission, requestPermission] = useCameraPermissions(); const [qrPayload, setQrPayload] = useState(''); const [serialNumber, setSerialNumber] = useState('');
  const [name, setName] = useState(''); const [username, setUsername] = useState(''); const [password, setPassword] = useState(''); const [pin, setPin] = useState(''); const [token, setToken] = useState(''); const [passcode, setPasscode] = useState('');
  const [scanning, setScanning] = useState(false); const [saving, setSaving] = useState(false);
  const detectedProfile = useMemo(() => detectAuthenticationProfile({ source: draft.method ?? 'manual', qrPayload: qrPayload || undefined, model: undefined, manufacturer: undefined, protocol: 'unknown' }), [draft.method, qrPayload]);
  ...
  const fieldValue = (id: string): string => ({ name, username, password, pin, token, passcode })[id as keyof Record<string, string>] ?? '';
  const setFieldValue = (id: string, value: string) => { if (id === 'name') setName(value); else if (id === 'username') setUsername(value); else if (id === 'password') setPassword(value); else if (id === 'pin') setPin(value); else if (id === 'token') setToken(value); else if (id === 'passcode') setPasscode(value); };
  ...
  const save = async () => {
    ...
    try {
      ...
      setDraft((current) => ({ ...current, step: 'complete' })); Alert.alert('Camera added', `${camera.name} was saved securely on this phone.`); router.replace('/cctv-cameras');
    } catch (error) { Alert.alert('Could not add camera', error instanceof Error ? error.message : 'The camera could not be saved securely.'); } finally { setSaving(false); }
  };
  ...
  return <View style={[styles.root, { backgroundColor: colors.background }]}>
    ...
    <ScrollView contentContainerStyle={[styles.content, { paddingTop: insets.top + 8, paddingBottom: insets.bottom + 28 }]} keyboardShouldPersistTaps="handled">
      ...
      {draft.step === 'identify' && draft.method && <>
        ...
        {(draft.method === 'serial' || draft.method === 'manual') && <Field label="Serial Number" value={serialNumber} onChangeText={setSerialNumber} placeholder="Enter serial number" colors={colors} autoCapitalize="none" />}
        <Text style={[styles.sectionTitle, { color: colors.foreground }]}>Device Type</Text><View style={styles.grid}>{DEVICE_KINDS.map((item) => <Pressable key={item.id} accessibilityRole="radio" accessibilityState={{ selected: deviceKind === item.id }} onPress={() => setDeviceKind(item.id)} style={[styles.kindCard, { borderColor: deviceKind === item.id ? colors.primary : colors.border, backgroundColor: deviceKind === item.id ? colors.secondary : colors.card }]}><Feather name={item.icon as never} size={18} color={colors.primary} /><Text style={[styles.kindText, { color: colors.foreground }]}>{item.label}</Text></Pressable>)}</View>
        <Pressable accessibilityRole="button" accessibilityLabel="Continue to camera authentication" onPress={() => acceptIdentification(qrPayload, serialNumber)} style={[styles.primaryButton, { backgroundColor: colors.primary }]}><Text style={[styles.buttonText, { color: colors.primaryForeground }]}>Continue</Text><Feather name="arrow-right" size={18} color={colors.primaryForeground} /></Pressable>
      </>}
      {draft.step === 'credentials' && <><Text style={[styles.sectionTitle, { color: colors.foreground }]}>Authentication</Text><Text style={[styles.note, { color: colors.mutedForeground }]}>Detected method: {detectedProfile.id.replaceAll('_', ' ')} ({detectedProfile.confidence}).</Text>{authFields.map((field) => <Field key={field.id} label={field.label} value={fieldValue(field.id)} onChangeText={(value) => setFieldValue(field.id, value)} placeholder={field.label} colors={colors} secureTextEntry={Boolean(field.secret)} autoCapitalize={field.id === 'name' ? 'sentences' : 'none'} />)}<Pressable accessibilityRole="button" accessibilityLabel="Add camera securely" disabled={saving} onPress={() => void save()} style={[styles.primaryButton, { backgroundColor: colors.primary, opacity: saving ? 0.6 : 1 }]}><Feather name="save" size={18} color={colors.primaryForeground} /><Text style={[styles.buttonText, { color: colors.primaryForeground }]}>{saving ? 'Adding Camera…' : 'Add Camera'}</Text></Pressable></>}
...
function Field({ label, value, onChangeText, placeholder, colors, secureTextEntry = false, autoCapitalize = 'sentences' }: { label: string; value: string; onChangeText: (value: string) => void; placeholder: string; colors: ReturnType<typeof useColors>; secureTextEntry?: boolean; autoCapitalize?: 'none' | 'sentences' }) { return <View style={styles.field}><Text style={[styles.label, { color: colors.foreground }]}><Text>{label}</Text></Text><TextInput accessibilityLabel={label} value={value} onChangeText={onChangeText} placeholder={placeholder} placeholderTextColor={colors.mutedForeground} secureTextEntry={secureTextEntry} autoCapitalize={autoCapitalize} style={[styles.input, { color: colors.foreground, backgroundColor: colors.card, borderColor: colors.border }]} /></View>; }
const styles = StyleSheet.create({ root: { flex: 1 }, content: { paddingHorizontal: 18, gap: 13 }, title: { fontSize: 24, fontFamily: 'Inter_700Bold' }, subtitle: { fontSize: 11, lineHeight: 17 }, progressRow: { flexDirection: 'row', gap: 6 }, progressBar: { height: 5, flex: 1, borderRadius: 4 }, sectionTitle: { fontSize: 14, fontFamily: 'Inter_700Bold', marginTop: 4 }, stack: { gap: 9 }, optionCard: { minHeight: 76, borderWidth: 1, borderRadius: 16, padding: 13, flexDirection: 'row', alignItems: 'center', gap: 11 }, optionIcon: { width: 44, height: 44, borderRadius: 13, alignItems: 'center', justifyContent: 'center' }, copy: { flex: 1 }, optionTitle: { fontSize: 15, fontFamily: 'Inter_700Bold' }, note: { fontSize: 10.5, lineHeight: 16 }, scannerWrap: { gap: 8 }, scanner: { width: '100%', height: 250, borderRadius: 18, overflow: 'hidden' }, scanFrame: { position: 'absolute', width: '66%', height: 165, left: '17%', top: 42, borderWidth: 3, borderColor: '#FFFFFF', borderRadius: 20 }, guidance: { fontSize: 13, fontFamily: 'Inter_700Bold', textAlign: 'center' }, success: { fontSize: 12, fontFamily: 'Inter_700Bold' }, grid: { flexDirection: 'row', flexWrap: 'wrap', gap: 8 }, kindCard: { width: '48%', minHeight: 68, borderWidth: 1, borderRadius: 14, padding: 11, justifyContent: 'center', gap: 6 }, kindText: { fontSize: 11, fontFamily: 'Inter_700Bold' }, field: { gap: 6 }, label: { fontSize: 11, fontFamily: 'Inter_700Bold' }, input: { minHeight: 48, borderWidth: 1, borderRadius: 13, paddingHorizontal: 13, fontSize: 13 }, primaryButton: { minHeight: 50, borderRadius: 14, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 9, paddingHorizontal: 16 }, buttonText: { fontSize: 12, fontFamily: 'Inter_700Bold' }, secondaryButton: { minHeight: 46, borderWidth: 1, borderRadius: 13, alignItems: 'center', justifyContent: 'center', flexDirection: 'row', gap: 8, paddingHorizontal: 16 }, secondaryText: { fontSize: 11, fontFamily: 'Inter_600SemiBold' }, completeCard: { borderWidth: 1, borderRadius: 18, padding: 24, alignItems: 'center', gap: 9 } });