...
export default function MergePdfScreen() {
  ...
  async function merge() {
    if (items.length < 2) {
      setStatus('Select at least two PDF files.');
      return;
    }
    setBusy(true);
    setResult(null);
    ...
    try {
      ...
      setStatus(error instanceof Error ? error.message : 'Could not merge these PDFs.');
    } finally {
      setBusy(false);
    }
  }
    ...
    <View style={[styles.root, { backgroundColor: colors.background }]}>
      ...
      {busy ? (
        <View style={styles.loading}>
          ...
          <Text accessibilityRole="header" style={[styles.loadingTitle, { color: colors.foreground }]}>Merging PDFs</Text>
          <Text accessibilityLiveRegion="polite" style={[styles.loadingText, { color: colors.mutedForeground }]}>Combining the selected PDF files into one document.</Text>
        </View>
      ) : (
        <ScrollView contentContainerStyle={{ paddingTop: insets.top + 18, paddingBottom: insets.bottom + 40 }} showsVerticalScrollIndicator={false}>
          <View style={styles.header}>
            <View style={[styles.icon, { backgroundColor: colors.secondary }]}><MaterialCommunityIcons name="file-document-multiple-outline" size={28} color={colors.primary} /></View>
            <View style={styles.copy}><Text accessibilityRole="header" style={[styles.title, { color: colors.foreground }]}>Merge PDF</Text><Text style={[styles.subtitle, { color: colors.mutedForeground }]}>Combine two or more PDF files into a single PDF.</Text></View>
          </View>
          <Pressable accessibilityRole="button" accessibilityLabel="Choose PDF files" onPress={() => void pickPdfs()} style={({ pressed }) => [styles.pick, { backgroundColor: colors.card, borderColor: colors.border }, pressed && styles.pressed]}>
            <Feather name="file-plus" size={20} color={colors.primary} />
            <View style={styles.pickCopy}><Text style={[styles.pickTitle, { color: colors.foreground }]}>Choose PDF files</Text><Text style={[styles.pickDetail, { color: colors.mutedForeground }]}>{items.length ? `${items.length} selected` : 'Select multiple PDFs'}</Text></View>
