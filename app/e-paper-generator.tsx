...
export default function EPaperGeneratorScreen() {
  ...
  const [status, setStatus] = useState('Ready. Use one paste or multiple stories — both are supported.');
  ...
  const commit = (next: EPaperDocument) => { setHistory((current) => pushHistory(current, doc)); setDoc(next); };
  ...
  function generate() {
    const chunks = normalizeContentInputs(singlePaste, separate);
    ...
    const next = generateEPaperLayout({ ...doc, title: title.trim() || 'My E-Paper', publisher: publisher.trim() || 'Nexus Plus' }, { title: title.trim() || 'My E-Paper', intro: intro.trim(), sections: generatedSections });
    commit(next);
    setStatus(`${chunks.length} story${chunks.length === 1 ? '' : 'ies'} classified and ${matchedImages.length} image${matchedImages.length === 1 ? '' : 's'} matched locally.`);
    setSelected(null);
  }
  ...
  function doUndo() { const result = undo(history, doc); setHistory(result.history); setDoc(result.document); setSelected(null); }
  function doRedo() { const result = redo(history, doc); setHistory(result.history); setDoc(result.document); setSelected(null); }
  ...
  async function share() {
    ...
    try {
      ...
      setStatus('Share will use the exported e-paper file. No publishing API is connected.');
    ...
    <View style={[styles.root, { backgroundColor: colors.background }]}> 
      ...
      <ScrollView contentContainerStyle={{ paddingTop: insets.top + 12, paddingBottom: insets.bottom + 40 }} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
        ...
        <View style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
          ...
          <Pressable accessibilityRole="button" accessibilityLabel="Add another story" onPress={addStory} style={[styles.secondaryButton, { backgroundColor: colors.secondary, borderColor: colors.border }]}><Feather name="plus" size={17} color={colors.primary} /><Text style={[styles.secondaryText, { color: colors.foreground }]}>Add Another Story</Text></Pressable>
          ...
          <Text style={[styles.label, { color: colors.foreground }]}>Intro</Text><TextInput accessibilityLabel="Intro" value={intro} onChangeText={setIntro} multiline style={[styles.input, { backgroundColor: colors.background, borderColor: colors.border, color: colors.foreground }]} />
          <Pressable accessibilityRole="button" accessibilityLabel="Add images and content files" onPress={() => void addAssets()} style={[styles.secondaryButton, { backgroundColor: colors.secondary, borderColor: colors.border }]}><Feather name="upload" size={18} color={colors.primary} /><Text style={[styles.secondaryText, { color: colors.foreground }]}>Add Images / Content Files</Text></Pressable>
          <Pressable accessibilityRole="button" accessibilityLabel="Generate e-paper automatically" onPress={generate} style={[styles.primaryButton, { backgroundColor: colors.primary }]}><MaterialCommunityIcons name="auto-fix" size={19} color={colors.primaryForeground} /><Text style={[styles.primaryText, { color: colors.primaryForeground }]}>Generate Automatically</Text></Pressable>
        </View>
        ...
        <View style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
          ...
          <Text style={[styles.helper, { color: colors.mutedForeground }]}>The page model is resolution-independent, so layout coordinates do not depend on the phone screen size.</Text>
        </View>

        <View style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
          <Text style={[styles.sectionTitle, { color: colors.foreground }]}>3. Element editor</Text>
          {selectedElement ? <>
            <Text style={[styles.helper, { color: colors.mutedForeground }]}>Selected: {selectedElement.type}. Edit position, size, text and stacking without regenerating the document.</Text>
          ...
          </> : <Text style={[styles.helper, { color: colors.mutedForeground }]}>Generate the e-paper first. The document model supports element-level editing and history.</Text>}
        </View>
...
const styles = StyleSheet.create({
  ...
  card: { marginHorizontal: 16, borderWidth: 1, borderRadius: 17, padding: 14, marginBottom: 12 },
  sectionHeader: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
  sectionTitle: { fontSize: 15, fontFamily: 'Inter_700Bold', marginBottom: 12 },
  historyRow: { flexDirection: 'row', gap: 7 },
  iconButton: { width: 38, height: 38, borderWidth: 1, borderRadius: 11, alignItems: 'center', justifyContent: 'center' },
  label: { fontSize: 11.5, fontFamily: 'Inter_700Bold', marginBottom: 6, marginTop: 4 },
  input: { minHeight: 44, borderWidth: 1, borderRadius: 11, paddingHorizontal: 11, paddingVertical: 9, fontSize: 12.5, marginBottom: 9 },
  largeInput: { minHeight: 160, textAlignVertical: 'top' },
  mediumInput: { minHeight: 100, textAlignVertical: 'top' },
  orRow: { flexDirection: 'row', alignItems: 'center', gap: 8, marginVertical: 8 },
  rule: { flex: 1, height: 1 },
  orText: { fontSize: 8.5, fontFamily: 'Inter_700Bold', letterSpacing: 0.6 },
  storyBox: { borderWidth: 1, borderRadius: 13, padding: 10, marginBottom: 9 },
  remove: { flexDirection: 'row', alignItems: 'center', gap: 7, alignSelf: 'flex-start', paddingVertical: 5 },
  secondaryButton: { minHeight: 46, borderWidth: 1, borderRadius: 12, alignItems: 'center', justifyContent: 'center', flexDirection: 'row', gap: 8, marginTop: 3 },
  secondaryText: { fontSize: 12.5, fontFamily: 'Inter_700Bold' },
  primaryButton: { minHeight: 49, borderRadius: 13, alignItems: 'center', justifyContent: 'center', flexDirection: 'row', gap: 9, marginTop: 9 },
  primaryText: { fontSize: 13.5, fontFamily: 'Inter_700Bold' },
  pillRow: { flexDirection: 'row', flexWrap: 'wrap', gap: 7, marginBottom: 8 },
  pill: { minWidth: 38, height: 38, borderWidth: 1, borderRadius: 10, alignItems: 'center', justifyContent: 'center' },
  pillWide: { minHeight: 38, paddingHorizontal: 10, borderWidth: 1, borderRadius: 10, alignItems: 'center', justifyContent: 'center' },
  pillText: { fontSize: 10.5, fontFamily: 'Inter_700Bold' },
  helper: { fontSize: 10.5, lineHeight: 16, marginTop: 5 },
  numericRow: { flexDirection: 'row', gap: 7, marginTop: 8 },
  smallInput: { flex: 1, minWidth: 54, height: 40, borderWidth: 1, borderRadius: 10, paddingHorizontal: 8, fontSize: 11 },
  actionRow: { flexDirection: 'row', gap: 8, marginTop: 10 },
  action: { flex: 1, minHeight: 44, borderWidth: 1, borderRadius: 11, alignItems: 'center', justifyContent: 'center', flexDirection: 'row', gap: 7 },
  actionText: { fontSize: 11.5, fontFamily: 'Inter_700Bold' },
  previewStage: { alignItems: 'center', justifyContent: 'center', minHeight: 320, borderRadius: 14, padding: 10 },
  paper: { borderWidth: 1, padding: 11 },
  paperTitle: { fontSize: 18, fontFamily: 'Inter_800ExtraBold', textAlign: 'center' },
  paperMeta: { fontSize: 7, textAlign: 'center', marginTop: 3, marginBottom: 7 },
  previewColumns: { flexDirection: 'row', gap: 4, flex: 1 },
  previewColumn: { flex: 1, paddingTop: 3 },
  fakeLine: { height: 3, borderRadius: 2, opacity: 0.5, marginBottom: 4 },
  fakeLineShort: { width: '70%', height: 3, borderRadius: 2, marginBottom: 5 },
  fakeImage: { width: '100%', height: 44, borderRadius: 3, marginBottom: 6 },
  status: { marginTop: 9, fontSize: 10.5, lineHeight: 16 },
});