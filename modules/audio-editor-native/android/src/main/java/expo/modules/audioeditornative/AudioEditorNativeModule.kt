...
class AudioEditorNativeModule : Module() {
  override fun definition() = ModuleDefinition {
    ...
    AsyncFunction("audioEffectAdvanced") { inputPath: String, outputPath: String, effect: String, preset: String, amount: Double, delayMs: Double, feedback: Double, promise: Promise ->
      try {
        val context = requireNotNull(appContext.reactContext) { "Audio editor context is unavailable." }
        promise.resolve(AudioEffectProcessor.processReverbDelay(context, inputPath, outputPath, preset, amount, delayMs, feedback).let { mapOf("outputPath" to it.outputPath, "durationMs" to it.durationMs, "sampleRate" to it.sampleRate, "channels" to it.channels, "mimeType" to it.mimeType) })
      } catch (error: Exception) { promise.reject("AUDIO_ADVANCED_EFFECT_FAILED", error.message ?: "Unable to apply advanced audio effect", error) }
    }
