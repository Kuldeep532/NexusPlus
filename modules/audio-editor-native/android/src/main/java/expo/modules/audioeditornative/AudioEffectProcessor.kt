...
internal object AudioEffectProcessor {
  ...
  private fun decodePcm(context: Context, inputPath: String): Decoded {
    ...
    try {
      ...
      val inputFormat = format!!; val rate = inputFormat.getInteger(MediaFormat.KEY_SAMPLE_RATE); val channels = inputFormat.getInteger(MediaFormat.KEY_CHANNEL_COUNT); val mime = inputFormat.getString(MediaFormat.KEY_MIME) ?: error("Audio codec MIME type is missing.")
      extractor.selectTrack(track); val decoder = MediaCodec.createDecoderByType(mime); decoder.configure(inputFormat, null, null, 0); decoder.start()
      ...
      try {
        while (!outputDone) {
          if (!inputDone) {
            ...
            if (idx >= 0) {
              val buffer = decoder.getInputBuffer(idx) ?: error("Decoder input unavailable."); buffer.clear(); val size = extractor.readSampleData(buffer, 0)
          ...
          when (val oi = decoder.dequeueOutputBuffer(info, 10_000)) {
            MediaCodec.INFO_TRY_AGAIN_LATER, MediaCodec.INFO_OUTPUT_FORMAT_CHANGED -> Unit
              ...
              val buffer = decoder.getOutputBuffer(oi) ?: error("Decoder output unavailable."); if (info.size > 0) { buffer.position(info.offset); buffer.limit(info.offset + info.size); while (buffer.remaining() >= 2) samples.add(buffer.short.toInt() / 32768f) }
              decoder.releaseOutputBuffer(oi, false); if ((info.flags and MediaCodec.BUFFER_FLAG_END_OF_STREAM) != 0) outputDone = true
            }
          }
        }
  ...
  private fun echo(input: FloatArray, rate: Int, channels: Int, amount: Double): FloatArray {
    val delay = (rate * (0.16 + 0.18 * amount)).toInt(); val out = input.copyOf(); val feedback = (0.18 + amount * 0.3).toFloat()
  ...
  private fun encodeAac(samples: FloatArray, rate: Int, channels: Int, path: String): String {
    val file = File(path); file.parentFile?.mkdirs(); val format = MediaFormat.createAudioFormat("audio/mp4a-latm", rate, channels); format.setInteger(MediaFormat.KEY_BIT_RATE, minOf(192_000, maxOf(64_000, 64_000 * channels))); format.setInteger(MediaFormat.KEY_AAC_PROFILE, android.media.MediaCodecInfo.CodecProfileLevel.AACObjectLC)
    val encoder = MediaCodec.createEncoderByType("audio/mp4a-latm"); val muxer = MediaMuxer(file.absolutePath, MediaMuxer.OutputFormat.MUXER_OUTPUT_MPEG_4); val info = MediaCodec.BufferInfo(); var track = -1; var started = false; var offset = 0; var eos = false
    try { encoder.configure(format, null, null, MediaCodec.CONFIGURE_FLAG_ENCODE); encoder.start(); var done = false; while (!done) {
      if (!eos) { val idx = encoder.dequeueInputBuffer(10_000); if (idx >= 0) { val buffer = encoder.getInputBuffer(idx) ?: error("Encoder input unavailable."); buffer.clear(); val cap = buffer.remaining() / (2 * channels); val total = samples.size / channels; val remain = total - offset; val count = minOf(cap, remain); if (count <= 0) { encoder.queueInputBuffer(idx, 0, 0, offset.toLong() * 1_000_000L / rate, MediaCodec.BUFFER_FLAG_END_OF_STREAM); eos = true } else { var bytes = 0; for (f in 0 until count) for (c in 0 until channels) { val s = (samples[(offset + f) * channels + c].coerceIn(-1f, 1f) * 32767f).toInt().toShort(); buffer.put((s.toInt() and 255).toByte()); buffer.put(((s.toInt() shr 8) and 255).toByte()); bytes += 2 }; encoder.queueInputBuffer(idx, 0, bytes, offset.toLong() * 1_000_000L / rate, 0); offset += count } } }
      when (val oi = encoder.dequeueOutputBuffer(info, 10_000)) { MediaCodec.INFO_TRY_AGAIN_LATER -> Unit; MediaCodec.INFO_OUTPUT_FORMAT_CHANGED -> if (!started) { track = muxer.addTrack(encoder.outputFormat); muxer.start(); started = true }; else -> if (oi >= 0) { val buffer = encoder.getOutputBuffer(oi) ?: error("Encoder output unavailable."); if ((info.flags and MediaCodec.BUFFER_FLAG_CODEC_CONFIG) != 0) info.size = 0; if (info.size > 0) { buffer.position(info.offset); buffer.limit(info.offset + info.size); require(started); muxer.writeSampleData(track, buffer, info) }; encoder.releaseOutputBuffer(oi, false); if ((info.flags and MediaCodec.BUFFER_FLAG_END_OF_STREAM) != 0) done = true } }
    } } finally { runCatching { encoder.stop() }; encoder.release(); if (started) muxer.stop(); muxer.release() }; return file.absolutePath
  }
}
