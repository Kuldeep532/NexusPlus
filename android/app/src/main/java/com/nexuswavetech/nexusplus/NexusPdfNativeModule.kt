...
class NexusPdfNativeModule(private val reactContext: ReactApplicationContext) : ReactContextBaseJavaModule(reactContext) {
    ...
    @ReactMethod
    fun pdfToWord(inputPath: String, outputPath: String, promise: Promise) = convertDocs(inputPath, outputPath, "pdf", "docx", promise)

    @ReactMethod
    fun wordToPdf(inputPath: String, outputPath: String, promise: Promise) = convertDocs(inputPath, outputPath, "docx", "pdf", promise)
        ...
        runCatching {
            ...
            PDDocument.load(input).use { document ->
                ...
                pages.map { pageNumber ->
                    val probe = renderSafely(renderer, document, pageNumber - 1, safeDpi)
                    try {
    ...
    fun combineImages(inputPaths: ReadableArray, outputPath: String, format: String, quality: Int, promise: Promise) {
        runCatching {
            ...
            try {
                ...
                val totalHeightLong = decoded.sumOf { it.height.toLong() }
                ...
                require(maxWidth.toLong() * totalHeight.toLong() * 4L <= maxSafeBitmapBytes()) { "Selected pages cannot fit safely into one image on this device." }
                val combined = Bitmap.createBitmap(maxWidth, totalHeight, Bitmap.Config.ARGB_8888)
                try {
                    val canvas = Canvas(combined); canvas.drawColor(android.graphics.Color.WHITE); var top = 0f
                    decoded.forEach { bitmap -> canvas.drawBitmap(bitmap, 0f, top, null); top += bitmap.height }
                    val output = File(outputPath); output.parentFile?.mkdirs()
                    FileOutputStream(output).use { stream ->
                        val compressed = if (normalized == "png") Bitmap.CompressFormat.PNG else Bitmap.CompressFormat.JPEG
                        require(combined.compress(compressed, quality.coerceIn(1, 100), stream)) { "Unable to encode combined image." }
                    }
                    output.absolutePath
        ...
        runCatching {
            ...
            PDDocument.load(input).use { source ->
                ...
                for (index in 0 until pageRanges.size()) {
                    val range = requireArrayString(pageRanges, index).trim(); val match = Regex("^(\\d+)(?:-(\\d+))?$").matchEntire(range) ?: throw IllegalArgumentException("Invalid page range: $range")
                    val start = match.groupValues[1].toInt(); val end = if (match.groupValues[2].isEmpty()) start else match.groupValues[2].toInt(); require(start in 1..pageCount && end in 1..pageCount) { "Page range is outside the document: $range" }
                    val first = minOf(start, end); val last = maxOf(start, end)
                    PDDocument().use { part -> for (pageIndex in first - 1 until last) part.importPage(source.getPage(pageIndex)); val output = File(outputDir, "part-${index + 1}-${first}-${last}.pdf"); FileOutputStream(output).use { part.save(it) }; results += output.absolutePath }
                }
                results
            }
    ...
    fun compress(inputPath: String, outputPath: String, quality: Int, promise: Promise) {
        runCatching { ensurePdfBoxInitialized(); val output = File(outputPath); output.parentFile?.mkdirs(); PDDocument.load(File(requireReadablePath(inputPath))).use { document -> FileOutputStream(output).use { document.save(it) } }; output.absolutePath }.onSuccess { promise.resolve(it) }.onFailure { promise.reject("PDF_COMPRESS", it.message, it) }
    }
    ...
    private fun convertDocs(inputPath: String, outputPath: String, inputExtension: String, outputExtension: String, promise: Promise) {
        try {
            val future = DocsEngineRunner.submit(inputPath, outputPath, inputExtension, outputExtension)
            val result = DocsEngineRunner.await(future)
            promise.resolve(result)
        } catch (error: Throwable) {
            promise.reject("DOCS_CONVERSION", error.message ?: "Document conversion failed.", error)
        }
    }
    ...
    private fun requireReadablePath(path: String): String { require(path.isNotBlank() && path.length <= 4096) { "Invalid PDF input path." }; require(!path.contains('\u0000') && !path.contains('\r') && !path.contains('\n')) { "Invalid PDF input path." }; val file = File(path); require(file.isFile && file.canRead()) { "PDF input is not readable." }; return file.absolutePath }
    private fun sanitizePathSegment(value: String, fallback: String): String { val normalized = value.trim().replace(Regex("[^a-zA-Z0-9._-]"), "_").take(80); return normalized.ifEmpty { fallback } }
    private fun sanitizeFilename(value: String, fallback: String): String { val normalized = value.trim().replace(Regex("[/\\\\]"), "_").replace(Regex("[^a-zA-Z0-9._ -]"), "_").trim().take(160); return normalized.ifEmpty { fallback } }
    private fun requireArrayString(array: ReadableArray, index: Int): String = array.getString(index) ?: throw IllegalArgumentException("Invalid path value.")
    ...
    companion object {
        private const val MAX_PDF_PAGE_POINTS = 14_000f
        private const val MAX_BITMAP_DIMENSION = 8_000
    }
}
