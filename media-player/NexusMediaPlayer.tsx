...
export function NexusMediaPlayer({ initialItems = [], onBack }: Props) {
  ...
  if (screen === 'player') {
    return <View style={styles.root}>
      ...
      <View style={styles.controls}><Button label="Shuffle" onPress={player.toggleShuffle} text="🔀" selected={player.state.shuffle} /><Button label="Repeat" onPress={player.cycleRepeat} text="↻" /><Button label="Volume down" onPress={() => { if (current?.kind==='audio') { persistent.pause(); } else player.setVolume(player.state.volume - .1); }} text="🔉" /><Button label="Volume up" onPress={() => { if (current?.kind!=='audio') player.setVolume(player.state.volume + .1); }} text="🔊" /></View>
