    ...
    <View style={[styles.root, { backgroundColor: colors.background }]}> 
      ...
      <ScrollView contentContainerStyle={{ paddingTop: insets.top + 8, paddingBottom: insets.bottom + 40 }}>
        ...
        <View style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
          <View style={[styles.row, styles.divider, { borderBottomColor: colors.border }]}><MaterialCommunityIcons name="account-circle-outline" size={21} color={colors.primary} /><View style={styles.copy}><Text style={[styles.label, { color: colors.mutedForeground }]}>Name</Text><Text style={[styles.value, { color: colors.foreground }]}>{user?.displayName || 'Not available'}</Text></View></View>
          <View style={[styles.row, styles.divider, { borderBottomColor: colors.border }]}><MaterialCommunityIcons name="email-outline" size={21} color={colors.primary} /><View style={styles.copy}><Text style={[styles.label, { color: colors.mutedForeground }]}>Email</Text><Text style={[styles.value, { color: colors.foreground }]}>{user?.email || 'Not available'}</Text></View></View>
...
const styles = StyleSheet.create({
  ...
  avatar: { width: 88, height: 88, borderRadius: 30, alignItems: 'center', justifyContent: 'center', marginBottom: 14 },
  avatarText: { fontSize: 30, fontFamily: 'Inter_700Bold' },
  name: { fontSize: 24, fontFamily: 'Inter_700Bold', textAlign: 'center' },
  signedIn: { fontSize: 12, fontFamily: 'Inter_400Regular', marginTop: 5 },
  card: { marginHorizontal: 20, borderRadius: 18, borderWidth: 1, paddingHorizontal: 14, marginBottom: 22 },
  row: { minHeight: 68, flexDirection: 'row', alignItems: 'center', gap: 12 },
  divider: { borderBottomWidth: 1 },
  copy: { flex: 1 },
  label: { fontSize: 10, fontFamily: 'Inter_500Medium', marginBottom: 4 },
  value: { fontSize: 14, fontFamily: 'Inter_700Bold' },
  sectionTitle: { marginHorizontal: 20, marginBottom: 10, fontSize: 14, fontFamily: 'Inter_700Bold' },
  action: { minHeight: 72, flexDirection: 'row', alignItems: 'center', gap: 12 },
  description: { fontSize: 11, lineHeight: 16, fontFamily: 'Inter_400Regular', marginTop: 3 },
});
