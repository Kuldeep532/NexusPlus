...
export default function QRGeneratorScreen() {
  ...
  const value = useMemo(() => {
    ...
    if (tab === 'wifi') {
      ...
      const enterprise = security === 'WPA3-Enterprise' && enterpriseIdentity.trim() ? `;identity:${escapeWifi(enterpriseIdentity.trim())}` : '';
      return `WIFI:T:${auth};S:${escapeWifi(ssid.trim())};P:${escapeWifi(password)};H:${hidden ? 'true' : 'false'}${enterprise};;`;
    }
    if (tab === 'upi') {
  ...
  return (
    <ScrollView style={[styles.screen, { backgroundColor: colors.background }]} contentContainerStyle={{ paddingTop: insets.top + 12, paddingBottom: insets.bottom + 50 }} keyboardShouldPersistTaps="handled">
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.tabs}>
        {tabs.map((item) => <Pressable key={item.key} accessibilityRole="tab" accessibilityState={{ selected: tab === item.key }} onPress={() => setTab(item.key)} style={[styles.tab, { borderColor: colors.border, backgroundColor: tab === item.key ? colors.primary : colors.card }]}><Text style={{ color: tab === item.key ? colors.primaryForeground : colors.foreground, fontFamily: 'Inter_600SemiBold', fontSize: 12 }}>{item.label}</Text></Pressable>)}
      </ScrollView>
