...
export default function BatteryAnnouncerScreen() {
  ...
  useEffect(() => {
    ...
    const stateSub = Battery.addBatteryStateListener(async ({ batteryState }) => {
      ...
      if (settings.enabled && settings.announceStatusChanges) {
        const phrase = getBatteryPhrase(level, getBatteryStatus(batteryState), prefs.featureTtsLanguage);
  ...
  }, [settings.enabled, settings.announceStatusChanges, level]);

  const announceNow = async () => {
    await readCurrentBattery(language, speakFeatureText);
